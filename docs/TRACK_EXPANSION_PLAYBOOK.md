...
施設追加は公開して終わりではない。2026-09-26に既存133施設をLuna xhighで初回試走し、以後は全件を固定周期でAI再判定せず、次の事象で再確認する。費用と限界は [`../research/track-expansion/reverification-initial-trial-2026-09-26.md`](../research/track-expansion/reverification-initial-trial-2026-09-26.md) を参照する。

- availability: 既存の日次range生成。取得・解析失敗は該当施設だけunknownへ降格する。
- 公式施設・個人利用資料: 毎週URLを直接取得し、本文・redirectの変化、読取不能からの復旧を検知する。HTTP取得だけではOpenAI検索料は発生しない。
- 期間付き記述: 公開値の年月日と期間終了翌日を確認キューへ入れる。公式ページが更新されない場合も判定する。
- 新しい公式資料: 施設ごとに前回探索から365日経ったらWeb検索で候補を探す。一度の週次処理で最大20施設を進め、14日以上読めない公式URLは優先して探索する。検索候補は同一公式hostまたは既知公式ページからのlinkで関係を確認し、施設名を含む実ページを取得できたものだけ監視へ追加する。

変更候補はLuna xhighに公開値と取得済み公式本文を渡し、`null` は未記載、`available` は条件付きの個人利用資格を含むものとして判定する。日別の空き状況だけからstatic資格を変えない。値の型、資料中の原文引用、source URLの一致と独立した2回目のLuna xhigh判定を通った属性だけ小さなPRへ反映し、必須の `Node 24 validation` が成功してから自動マージする。根拠不足・矛盾はreportと再試行状態へ残す。全ての既知公式資料が28日間読めない場合だけ、個人利用statusを安全側の `unknown` へ戻す。施設自体を404だけで削除しない。取得資料のhash、変更根拠、API usage、実際の検索action数を履歴へ残し、暦年の保守的なAPI費推計が$4.80へ達したら新たなAI処理を停止する。
