...
| T09 | blocked by Stop condition | `pending` | R01-R06 local work and scoped bootstrap completed; `cdk deploy` stopped during changeset creation. | CloudFormation execution role lacks `ssm:GetParameters` for `/cdk-bootstrap/hnb659fds/version`. No guessed permission expansion; R07 and T10/Phase 4 not started. |
