...
| FF03 | complete | pending | `npm run check`; `npx vitest run scripts/migration/deploy-preview.test.mjs`; `PREVIEW_BASE_URL=https://d2via50thoheqm.cloudfront.net npm run test:e2e:preview`; `git diff --check`; `git status --short` | Restored C05's 64-case history, recorded exact RR01/FF commits, and separated RR03's initial 76/8 test-only failure from its final 84-case pass. No AWS deployment or invalidation was performed. |
