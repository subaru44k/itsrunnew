...
`master`向けPull RequestではGitHub Actionsの `Node 24 validation` が、`npm ci`、候補batch検証、Track Dataset検証、unit test、lint/type check、buildを同じ順序で実行します。通常のunit/build検証はcommit済みavailability baselineを使用し、続くdaily gateでは実収集・Chromeによるsmokeも実行します。外部sourceへのnetworkとPython 3（監視メールのmock検証）が必要ですが、AWS credentialsやSMTP secretsは不要です。
