<template>
  <v-container class="archive py-4">
    ...
    <section class="panel">
      <h2>世界大会の前後を読む</h2>
      <div class="stories">
        <article v-for="story in seasonStories" :key="story.year">
          <a :href="`#${story.year}`">{{ story.year }}</a><p>{{ story.text }}</p>
        </article>
      </div>
    </section>
...
const seasonStories = [
  {year:2026,text:'冬は米国・豪州・アジアの室内外9レースで土台を作り、春は関西実業団のオープン種目やMDCのペース役も担当。日本選手権後はホクレンを挟み、欧州DLへつないでいる。'},
  {year:2025,text:'室内5レースから世界室内へ。春はGrand Slam Trackを転戦しつつ金栗・GGPでペース役も務め、日本選手権、東京世界陸上へ。世界選手権後もEDION、ホノルル、久喜のゲストランまで走った。'},
  {year:2024,text:'室内、世界クロカン、国内GPを経て米国・欧州で五輪標準を追い、日本選手権4種目を走ってパリへ。五輪後もDL4戦、屋島2000m、熊本のロード、ホノルルまで継続した。'},
  {year:2023,text:'室内と豪州遠征から始まり、春は1000m・800m・1マイルを含む国内外の小規模戦を多数経験。ブダペスト世界陸上後もロードマイル、DL、国体、MDC、日体大、ホノルルへ続いた。'},
  {year:2022,text:'春の国内GPでは同日2種目も走り、米国遠征と日本選手権3種目を経てオレゴン世界陸上で800・1500・5000mに出場。秋は欧州、実業団、国体、MDC、ロード、駅伝へ展開した。'},
  {year:2021,text:'高松・明石・兵庫の記録会から始め、日本選手権3種目とホクレン3戦を経て東京五輪へ。五輪後も日体大、MDC、屋島、北九州、静岡、EDIONと12月まで高頻度で走った。'},
  {year:2020,text:'コロナ禍で本格始動は夏。ホクレン4戦で距離を変えながら状態を上げ、GGPで1500m日本記録。秋冬は日本選手権だけでなく神戸・静岡・屋島の記録会、年末の10kmロードまで走った。'},
];
