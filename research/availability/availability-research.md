...
2026-08-25に神奈川・千葉・埼玉の公式一次情報を確認した18候補を追加し、当時のTrack Datasetは51施設になった。2026-08-27に初期33施設を含む全51候補を遡及監査し、施設identityを確認できなかった1件を除外して50施設へ補正した。同日の関東網羅性batchで東京・千葉・神奈川の19施設、関西batchで大阪・兵庫・京都の20施設を追加し、現在は89施設である。既存collectorで意味まで安全に確定できる施設数は23、coverageは23/89（25.8%）。未対応施設は共通fallbackにより `unknown` となり、情報欠落を利用不可とは解釈しない。
