...
export async function readFirestoreSnapshot(adapter, config) {
  ...
  const rawSnapshot = canonicalize({ schemaVersion: 1, collections }, 'snapshot'); let normalized; try { normalized = normalizeFirestoreSnapshot(rawSnapshot, { allowLegacyStatusStrings: true }) } catch (error) { throw sanitizeSnapshotError(error) }
  const snapshot = canonicalize({ schemaVersion: 1, collections: rawSnapshot.collections.map((collection) => ({ ...collection, documents: collection.documents.map((document) => ({ ...document, data: { status: document.data.status.map((value) => typeof value === 'string' ? Number(value) : value) } })) })) }, 'snapshot')
