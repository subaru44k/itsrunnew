...
export function serializeExport(result) { if (!plain(result) || !plain(result.snapshot) || !plain(result.capture) || !(result.snapshotBytes instanceof Uint8Array) || !(result.captureBytes instanceof Uint8Array) || !exactSorted(result.capture, ['contexts', 'counts', 'databaseId', 'capturedAt', 'normalizedDataSha256', 'projectId', 'schemaVersion'])) fail('serialization'); const expectedSnapshot = canonicalBytes(result.snapshot, 'snapshot'); const expectedCapture = canonicalBytes(result.capture, 'capture'); if (JSON.stringify([...expectedSnapshot]) !== JSON.stringify([...result.snapshotBytes]) || JSON.stringify([...expectedCapture]) !== JSON.stringify([...result.captureBytes])) fail('serialization'); try { normalizeFirestoreSnapshot(result.snapshot) } catch { fail('serialization') }; return { snapshotBytes: expectedSnapshot, captureBytes: expectedCapture } }
...
export async function preflightOutput(name, options = {}) {
  ...
  return Object.freeze({ rootReal, target, workspaceReal })
}

export async function writeExportRun(result, outputName, options = {}) {
  validateRunName(outputName); const fs = { lstat, mkdir, mkdtemp, readFile, readdir, realpath, rename, rm, stat, writeFile, ...options.fs }; const handle = options.targetHandle ?? await preflightOutput(outputName, options); const root = handle.rootReal; const target = handle.target; const rereadRoot = await fs.realpath(root); if (rereadRoot !== root) fail('output-root'); const rootInfo = await fs.lstat(root); if (!rootInfo.isDirectory() || rootInfo.isSymbolicLink()) fail('output-root')
  ...
  try { temp = await fs.mkdtemp(join(root, `.${outputName}-tmp-`)); const tempReal = await fs.realpath(temp); if (!relative(root, tempReal) || relative(root, tempReal).startsWith(`..${sep}`)) fail('output'); await fs.writeFile(join(tempReal, 'snapshot.json'), serialized.snapshotBytes, { flag: 'wx' }); await fs.writeFile(join(tempReal, 'capture.json'), serialized.captureBytes, { flag: 'wx' }); const files = (await fs.readdir(tempReal)).sort(); if (files.length !== 2 || files[0] !== 'capture.json' || files[1] !== 'snapshot.json') fail('reread'); const rereadSnapshotBytes = new Uint8Array(await fs.readFile(join(tempReal, 'snapshot.json'))); const rereadCaptureBytes = new Uint8Array(await fs.readFile(join(tempReal, 'capture.json'))); if (JSON.stringify([...rereadSnapshotBytes]) !== JSON.stringify([...serialized.snapshotBytes]) || JSON.stringify([...rereadCaptureBytes]) !== JSON.stringify([...serialized.captureBytes])) fail('reread'); const rereadSnapshot = JSON.parse(new TextDecoder().decode(rereadSnapshotBytes)); const rereadCapture = JSON.parse(new TextDecoder().decode(rereadCaptureBytes)); const rereadNormalized = normalizeFirestoreSnapshot(rereadSnapshot); const expectedHash = sha256(canonicalBytes({ schemaVersion: 1, records: rereadNormalized }, 'normalized')); if (JSON.stringify(canonicalize(rereadCapture, 'capture')) !== JSON.stringify(canonicalize(result.capture, 'capture')) || expectedHash !== rereadCapture.normalizedDataSha256 || !exactSorted(rereadCapture, ['contexts', 'counts', 'databaseId', 'capturedAt', 'normalizedDataSha256', 'projectId', 'schemaVersion'])) fail('reread'); const beforeRenameRoot = await fs.realpath(root); if (beforeRenameRoot !== root) fail('output-root'); await fs.rename(tempReal, target); temp = null; return target } catch (error) { if (temp) { try { await fs.rm(temp, { recursive: true, force: true }) } catch {} }; throw sanitize(error, 'output') }
}
...
export async function runCli(argv = process.argv.slice(2), env = process.env, loader, options = {}) { const args = parseArgs(argv); if (args.has('help')) { usage(); return 0 }; if (!args.has('output')) return 2; const name = validateOutputName(args.get('output')); const targetHandle = await preflightOutput(name, options); const config = validateExportConfig({ projectId: PROJECT_ID, databaseId: DATABASE_ID, capturedAt: new Date().toISOString(), env }); const load = loader ?? (async () => { const app = await import('firebase-admin/app'); const firestore = await import('firebase-admin/firestore'); return { ...app, ...firestore } }); const sdk = await load(); let app; try { app = sdk.initializeApp({ credential: sdk.applicationDefault(), projectId: PROJECT_ID }); const db = sdk.getFirestore(app, DATABASE_ID); const adapter = { getDocument: async (path) => { const snap = await db.doc(path).get(); return { exists: snap.exists, data: snap.exists ? snap.data() : null } }, getCollection: async (path) => { const snap = await db.collection(path).get(); return { docs: snap.docs.map((doc) => ({ id: doc.id, data: doc.data() })) } } }; const result = await readFirestoreSnapshot(adapter, config); await writeExportRun(result, name, { ...options, targetHandle }); console.log('Read-only Firestore export completed.'); return 0 } finally { if (app && typeof sdk.deleteApp === 'function') await sdk.deleteApp(app).catch(() => {}) } }
