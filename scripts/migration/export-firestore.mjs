...
export function validateImpersonatedAdc(value) {
  if (!plain(value) || !exactSorted(value, ['type', 'service_account_impersonation_url', 'source_credentials']) || value.type !== 'impersonated_service_account' || !isImpersonationUrl(value.service_account_impersonation_url) || !plain(value.source_credentials) || !exactSorted(value.source_credentials, ['type', 'client_id', 'client_secret', 'refresh_token']) || value.source_credentials.type !== 'authorized_user' || !isSafeCredentialValue(value.source_credentials.client_id) || !isSafeCredentialValue(value.source_credentials.client_secret) || !isSafeCredentialValue(value.source_credentials.refresh_token)) fail('adc')
  return true
}
