...
describe('T14C deterministic Firestore comparator', () => {
  it('returns the exact zero-diff report and stable machine/human bytes', () => {
    const first = compareFirestoreArtifacts(records, transformed); const second = compareFirestoreArtifacts([...records].reverse(), transformed)
  ...
  it('reports changed cells, missing dates, and missing objects with sanitized coordinates', () => {
    ...
    const missingReport = compareFirestoreArtifacts([
      { slug: 'oda', date: '20240803', status: [2, 1, 0] }, { slug: 'oda', date: '20240801', status: [0, 1, 2] }, { slug: 'oda', date: '20240802', status: [1, 2, 0] },
    ], manifestFor([missingDate])).report
    expect(missingReport.mismatches.some((item) => item.kind === 'date' && item.date === '2024-08-02' && item.actual === 'missing')).toBe(true)
    const missingObject = manifestFor(transformed.objects.slice(1)); const objectReport = compareFirestoreArtifacts(records, missingObject).report
  ...
  it('reports extra objects and deterministic mismatch ordering', () => {
    ...
    const report = compareFirestoreArtifacts(records, manifestFor([...transformed.objects, extra.objects[0]])).report
  ...
  it('fails closed for invalid source, target integrity, and duplicate artifacts', () => {
    const sourceBefore = structuredClone(records); expect(compareFirestoreArtifacts([...records, records[0]], transformed).report.status).toBe('mismatch'); expect(records).toEqual(sourceBefore)
    expect(compareFirestoreArtifacts(records, { ...transformed, manifestBytes: new TextEncoder().encode('secret/path/body') }).report.mismatches[0].kind).toBe('integrity')
    expect(compareFirestoreArtifacts(records, { ...transformed, objects: [transformed.objects[0], transformed.objects[0]], manifest: transformed.manifest }).report.mismatches[0].kind).toBe('integrity')
    ...
    expect(compareFirestoreArtifacts(records, manifestFor([invalidBody, ...transformed.objects.slice(1)])).report.mismatches[0].kind).toBe('integrity')
    expect(compareFirestoreArtifacts(records, { ...transformed, manifest: { ...transformed.manifest, extra: 'secret' } }).report.mismatches[0].kind).toBe('integrity')
    const text = JSON.stringify(compareFirestoreArtifacts(records, { ...transformed, manifestBytes: new TextEncoder().encode('secret/path/body') }).report)
    expect(text).not.toMatch(/secret|path|body/i)
  })

  it('matches empty source and rejects tampered machine reports', () => {
    const empty = transformFirestoreRecords([], options); const result = compareFirestoreArtifacts([], empty)
