...
test('static resolver removes query, resolves index, and blocks decoded traversal', async () => {
  const root = await mkdtemp(join(tmpdir(), 'itsrun-static-')); await mkdir(join(root, 'nested')); await writeFile(join(root, 'index.html'), 'ok')
  assert.equal(resolveStaticPath('/?secret=1', root), join(root, 'index.html'))
  assert.equal(resolveStaticPath('/nested', root), join(root, 'nested', 'index.html'))
  for (const path of ['/../secret', '/%2e%2e/secret', '/%252e%252e/secret', '/..%5csecret', '/%5csecret']) assert.equal(resolveStaticPath(path, root), null)
})
