...
const headerValue = (headers, name) => normalizedHeader(headers, name)
...
export function createPlaywrightDataBrowser({ launcher = defaultBrowserLauncher, origin = DATA_CONSTANTS.apiOrigin, httpsPort, clock = () => Date.now(), sleep = ms => new Promise(resolve => setTimeout(resolve, ms)), timer = setTimeout, clearTimer = clearTimeout } = {}) {
  ...
  return {
    async setup({ username, password } = {}) {
      ...
      // Wait on the authenticated GET method only.  Validation below must see
      // malformed origin/path/status/type/cache responses instead of allowing a
      // too-specific predicate to leave a fake (or real) waiter unresolved.
      const getResponses = pages.map(page => page.waitForResponse(response => { try { return response.request().method() === 'GET' } catch { return false } }))
