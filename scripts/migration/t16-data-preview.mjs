...
export function classifyCurrentObject(object, baseline, expected = {}) {
  try {
    ...
    if (object.head.ETag === DATA_CONSTANTS.baselineEtag && object.head.VersionId === DATA_CONSTANTS.baselineVersionId && sha256(object.bytes) === (baseline.sha256 ?? DATA_CONSTANTS.baselineSha256) && object.parsed.tuple === DATA_CONSTANTS.before && exactEqual(object.parsed.document, baseline.parsed.document)) return { state: 'baseline', tuple: DATA_CONSTANTS.before }
...
export function createConcreteDataAdapters({ command, execFile, browser, browserLauncher = defaultBrowserLauncher, httpsPort, clock = () => Date.now(), sleep, timer = setTimeout, clearTimer = clearTimeout, fs: fsPort, randomBytesImpl = randomBytes } = {}) {
  ...
  return {
    ...
    async restore({ ifMatch, versionId, original = currentOriginal } = {}) {
      if (!original?.path || !ifMatch) fail('restore proof unavailable'); const checksum = createHash('sha256').update(original.bytes).digest('base64'); const result = await run('put-object', { inputPath: original.path, ifMatch, checksum }); const response = json(result.stdout); if (!strongEtag(response.ETag) || !response.VersionId || response.ETag === DATA_CONSTANTS.baselineEtag || response.ETag === ifMatch || response.VersionId === DATA_CONSTANTS.baselineVersionId || (versionId && response.VersionId === versionId)) fail('restore response mismatch'); const readback = await readObject({ allowTest: true }); if (readback.head.ETag !== response.ETag || readback.head.VersionId !== response.VersionId || readback.parsed.tuple !== DATA_CONSTANTS.before || sha256(readback.bytes) !== DATA_CONSTANTS.baselineSha256 || !exactEqual(readback.parsed.document, original.parsed.document) || readback.head.ContentType !== DATA_CONSTANTS.contentType || readback.head.CacheControl !== DATA_CONSTANTS.cacheControl) fail('restore readback mismatch'); return { status: 200, etag: response.ETag, versionId: response.VersionId, bytes: readback.bytes.length, sha256: sha256(readback.bytes), tuple: readback.parsed.tuple }
    },
    async cleanup({ restoreStatus = 'not-required', restoreAttempted = false, recoveryMaterialRetained = false } = {}) { let failed = false; try { await browserCall('cleanup', {}) } catch { failed = true } const dir = identity ? await protectedRoot() : root; if (identity) { await cognito('admin-remove-user-from-group', { UserPoolId: DATA_CONSTANTS.poolId, Username: identity, GroupName: 'admins' }, { root: dir }).catch(() => { failed = true }); await cognito('admin-delete-user', { UserPoolId: DATA_CONSTANTS.poolId, Username: identity }, { root: dir }).catch(() => { failed = true }) } let users = -1; let admins = -1; try { users = (await cognito('list-users', null, { jsonOutput: true }))?.Users?.length ?? -1 } catch { failed = true } try { admins = (await cognito('list-users-in-group', null, { jsonOutput: true }))?.Users?.length ?? -1 } catch { failed = true } if (users !== 0 || admins !== 0) failed = true; const removable = !failed && !recoveryMaterialRetained && (restoreStatus === 'passed' || (restoreStatus === 'not-required' && !restoreAttempted)); if (root && removable) { try { const parent = resolve('.artifacts/migration'); const parentInfo = await (fs.lstat ?? fs.stat)(parent); const rootInfo = await (fs.lstat ?? fs.stat)(root); const child = relative(parent, root); if ((parentInfo.mode & 0o777) !== 0o700 || parentInfo.isSymbolicLink?.() || (rootInfo.mode & 0o777) !== 0o700 || rootInfo.isSymbolicLink?.() || !child || child.startsWith('..') || child.includes('/') || !isAbsolute(root)) failed = true; else await fs.rm(root, { recursive: true, force: true }) } catch { failed = true } } if (failed) fail('cleanup failed'); return { users, admins } },
  }
}
