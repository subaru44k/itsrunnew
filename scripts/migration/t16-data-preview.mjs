...
function exactEqual(left, right) { return JSON.stringify(left) === JSON.stringify(right) }
...
export async function runDataRehearsal(adapters) {
  ...
  try {
    ...
    if (current.state !== 'test' || current.etag !== testEtag || current.versionId !== testVersionId || current.tuple !== DATA_CONSTANTS.after) throw new Error('current coupling mismatch')
...
export function createPlaywrightDataBrowser({ launcher = defaultBrowserLauncher, origin = DATA_CONSTANTS.apiOrigin, httpsPort, clock = () => Date.now(), sleep = ms => new Promise(resolve => setTimeout(resolve, ms)), timer = setTimeout, clearTimer = clearTimeout } = {}) {
  ...
  return {
    async setup({ username, password } = {}) {
      ...
      loaded = await Promise.all(getResponses.map(async response => { if (response.status() !== 200 || new URL(response.url()).origin !== new URL(origin).origin || (response.headers()['content-type'] ?? '').split(';')[0].trim().toLowerCase() !== 'application/json' || response.headers()['cache-control'] !== 'no-store') fail('authenticated GET contract'); const payload = await response.json(); if (Object.keys(payload ?? {}).sort().join('|') !== 'document|etag') fail('authenticated GET shape'); const document = payload.document; const etag = payload.etag; const parsed = parseSchedule(Buffer.from(JSON.stringify(document))); if (!strongEtag(etag) || parsed.tuple !== DATA_CONSTANTS.before) fail('authenticated GET proof'); return { document: clone(document), etag, tuple: parsed.tuple } }))
      return { contexts: 2 }
    },
    ...
    async submit(page, input, value, stale = false) { const index = pages.indexOf(page); const baseline = loaded[index]?.document ?? input.baselineDocument; if (!baseline) fail('page baseline unavailable'); const cell = page.locator(`#${DATA_CONSTANTS.date}-${DATA_CONSTANTS.slot}`); await cell.selectOption(String(value)); const matches = request => { try { const url = new URL(request.url()); return url.origin === new URL(origin).origin && url.pathname === DATA_CONSTANTS.apiPath && request.method() === 'PUT' } catch { return false } }; let requestCount = 0; const onRequest = request => { if (matches(request)) requestCount += 1 }; page.on?.('request', onRequest); const requestPromise = page.waitForRequest(matches); const responsePromise = page.waitForResponse(response => { try { const url = new URL(response.url()); return url.origin === new URL(origin).origin && url.pathname === DATA_CONSTANTS.apiPath && response.request().method() === 'PUT' } catch { return false } }); const comparisonPromise = stale ? page.waitForResponse(response => { try { const url = new URL(response.url()); return url.origin === new URL(origin).origin && url.pathname === DATA_CONSTANTS.apiPath && response.request().method() === 'GET' } catch { return false } }) : null; try { await page.getByRole('button', { name: /Save|保存/ }).click(); const request = await requestPromise; const response = await responsePromise; const headers = request.headers(); const body = request.postDataJSON(); if (headers['content-type']?.split(';')[0].trim().toLowerCase() !== 'application/json' || headers['if-match'] !== input.ifMatch || headers['if-none-match'] || !body || Object.keys(body).sort().join('|') !== 'days|schemaVersion|stadium|yearMonth' || body.schemaVersion !== 1 || body.stadium !== 'oda' || body.yearMonth !== '2026-08') fail('UI PUT contract'); validateOneCellDelta({ ...baseline, updatedAt: undefined }, { ...body, updatedAt: undefined }); const cacheControl = response.headers()['cache-control']?.split(';')[0].trim().toLowerCase() ?? ''; const result = await response.json(); if (response.status() === 409) { if (!stale || cacheControl !== 'no-store' || requestCount > 1) fail('conflict contract'); const comparison = await comparisonPromise; if (comparison.status() !== 200 || new URL(comparison.url()).origin !== new URL(origin).origin || comparison.headers()['content-type']?.split(';')[0].trim().toLowerCase() !== 'application/json' || comparison.headers()['cache-control']?.split(';')[0].trim().toLowerCase() !== 'no-store') fail('comparison contract'); const current = await comparison.json(); if (Object.keys(current ?? {}).sort().join('|') !== 'document|etag' || !strongEtag(current?.etag) || !accepted || current.etag !== accepted.etag || !exactEqual(current.document, accepted.document)) fail('comparison mismatch'); if (!page.getByRole || !(await page.getByRole('heading', { name: /conflict|競合/i }).isVisible().catch(() => false))) fail('conflict UI missing'); return { status: 409, etag: current.etag, cacheControl, puts: 1, retries: 0, tuple: value } } if (response.status() !== 200 || cacheControl !== 'no-store' || requestCount > 1 || Object.keys(result ?? {}).sort().join('|') !== 'document|etag|versionId' || typeof result.versionId !== 'string' || !result.versionId) fail('update contract'); validateOneCellDelta(baseline, result.document, { requireUpdatedAt: true }); const etag = result.etag; if (!strongEtag(etag)) fail('update etag'); const document = result.document; const parsed = parseSchedule(Buffer.from(JSON.stringify(document))); accepted = { document: clone(document), etag, versionId: result.versionId }; return { status: 200, etag, versionId: result.versionId, cacheControl, updatedAt: document.updatedAt, document, tuple: parsed.tuple, puts: 1 } } finally { page.off?.('request', onRequest) } },
