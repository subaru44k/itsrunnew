...
export async function validateProtectedMaterial({ fs, parent, run, file, bytes } = {}) {
  ...
  if ((parentInfo.mode & 0o777) !== 0o700 || parentInfo.isSymbolicLink?.() || !parentInfo.isDirectory?.() || (runInfo.mode & 0o777) !== 0o700 || runInfo.isSymbolicLink?.() || !runInfo.isDirectory?.() || !child || child.startsWith('..') || child.includes('/') || !child.startsWith('t16-data-') || (fileInfo.mode & 0o777) !== 0o600 || fileInfo.isSymbolicLink?.() || !fileInfo.isFile?.() || !fileChild || fileChild.startsWith('..') || fileChild.includes('/')) fail('protected path containment')
