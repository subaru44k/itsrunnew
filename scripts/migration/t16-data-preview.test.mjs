...
test('browser setup maps each production boundary to a closed substage and context', async () => {
  ...
  for (const stage of stages) for (const target of [0, 1]) {
    ...
    const response = { status: () => 200, url: () => `${DATA_CONSTANTS.apiOrigin}${DATA_CONSTANTS.apiPath}`, request: () => ({ method: () => 'GET' }), headers: () => ({ 'content-type': 'application/json', 'cache-control': 'no-store' }), json: async () => ({}) }
    const pages = [0, 1].map(index => ({ index, waitForResponse: () => Promise.resolve(response), goto: async () => {}, waitForURL: async () => { if (stage === 'manage-return' && index === target) throw new Error('TimeoutError') }, locator: () => ({}), getByRole: () => ({}) }))
    ...
    const browser = createPlaywrightDataBrowser({ launcher, responseTimeout: 20, browserRoleSession: async (_page) => { const index = roleCalls++; if (index !== target) return; if (stage === 'hosted-ui-redirect') throw new Error('hosted-ui-redirect-timeout'); if (stage === 'form-submission') throw { name: 'BrowserSubstageError', category: 'form-ambiguous', viewport: 'desktop' } }, signedInSentinel: async (_page) => { const index = sentinelCalls++; if (stage === 'signed-in-sentinel' && index === target) throw new Error('TimeoutError') }, getResponseValidator: async (_response) => { const index = validatorCalls++; if (stage === 'authenticated-api-response' && index === target) throw new Error('generic response failure'); return { document: {}, etag: DATA_CONSTANTS.baselineEtag, tuple: 0 } } })
    let thrown; try { await browser.setup({ username: 'opaque', password: 'opaque' }) } catch (error) { thrown = error }
    assert.deepEqual(thrown, { name: 'DataSetupSubstageError', category: stage, context: target === 0 ? 'first' : 'second', ...(stage === 'authenticated-api-response' ? { reason: 'transport-contract' } : {}) }); await browser.cleanup(); assert.equal(contextCloses, 2); assert.equal(browserCloses, 1); assert.doesNotMatch(JSON.stringify(thrown), /opaque|response failure|TimeoutError/)
  }
})
...
test('authenticated response diagnostics preserve only the closed reason and context', async () => {
  ...
  for (const reason of ['transport-contract', 'response-missing', 'body-read', 'body-shape', 'etag', 'schedule-schema', 'reserved-tuple']) for (const target of [0, 1]) {
    ...
    const response = index => {
      ...
      const document = invalid && reason === 'schedule-schema' ? { ...baseline, extra: true } : invalid && reason === 'reserved-tuple' ? { ...baseline, days: { [DATA_CONSTANTS.date]: [1, 1, 2] } } : baseline
      return { status: () => invalid && reason === 'transport-contract' ? 500 : 200, url: () => `${DATA_CONSTANTS.apiOrigin}${DATA_CONSTANTS.apiPath}`, request: () => ({ method: () => 'GET' }), headers: () => ({ 'content-type': 'application/json', 'cache-control': 'no-store' }), json: async () => { if (invalid && reason === 'body-read') throw new Error('raw body failure'); if (invalid && reason === 'body-shape') return { document, etag: DATA_CONSTANTS.baselineEtag, extra: true }; return { document, etag: invalid && reason === 'etag' ? 'weak' : DATA_CONSTANTS.baselineEtag } } }
    }
    const pages = [0, 1].map(index => ({ waitForResponse: async () => response(index), goto: async () => {}, waitForURL: async () => {}, locator: () => ({}), getByRole: () => ({}) })); const launcher = async () => ({ newContext: async () => ({ newPage: async () => pages[pageIndex++], close: async () => { contextCloses += 1 } }), close: async () => { browserCloses += 1 } })
    const browser = createPlaywrightDataBrowser({ launcher, responseTimeout: 20, browserRoleSession: async () => {}, signedInSentinel: async () => {} }); let thrown; try { await browser.setup({ username: 'opaque', password: 'opaque' }) } catch (error) { thrown = error }
    assert.deepEqual(thrown, { name: 'DataSetupSubstageError', category: 'authenticated-api-response', context: target === 0 ? 'first' : 'second', reason }); await browser.cleanup(); assert.equal(contextCloses, 2); assert.equal(browserCloses, 1); assert.doesNotMatch(JSON.stringify(thrown), /raw body|opaque|weak/)
  }
})
