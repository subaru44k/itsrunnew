...
const proof = {
  ...
  update: { status: 200, cacheControl: 'no-store', saved: true, tuple: 1, puts: 1 },
  stale: { status: 409, cacheControl: 'no-store', conflict: true, puts: 1, retries: 0, tuple: 1 },
  restore: { status: 200, etag: DATA_CONSTANTS.baselineEtag, versionId: 'restored-version', bytes: 501, sha256: DATA_CONSTANTS.baselineSha256, tuple: 0 },
...
function adapters({ fail = null, order = [], restoreArgs = [], loadArgs = [] } = {}) {
  ...
  return { preflight: make('preflight'), capture: make('capture'), setup: make('setup'), load: async input => { loadArgs.push(input); return make('load')(input) }, update: make('update'), readCurrent: async () => ({ state: 'test', etag: '"0123456789abcdef0123456789abcdef"', versionId: 'test-version', tuple: 1, document: { days: { '2026-08-09': [1, 1, 2] } } }), stale: make('stale'), poll: async input => { order.push(`poll-${input.expected}`); return { tuple: input.expected, attempts: 1 } }, restore: make('restore'), cleanup: make('cleanup') }
}
...
test('indeterminate exact test restores once with observed ETag and VersionId', async () => {
  const order = []; const restoreArgs = []; const base = adapters({ fail: 'update', order, restoreArgs }); base.readCurrent = async () => ({ state: 'test', etag: '"0123456789abcdef0123456789abcdef"', versionId: 'test-version', tuple: 1 }); const result = await runDataRehearsal(base)
  assert.equal(result.restoreStatus, 'passed'); assert.equal(restoreArgs.length, 1); assert.equal(restoreArgs[0].ifMatch, '"0123456789abcdef0123456789abcdef"'); assert.equal(restoreArgs[0].versionId, 'test-version')
})

test('post-stale ETag, VersionId, and document mismatches restore exactly once', async () => {
  for (const mismatch of ['etag', 'versionId', 'document']) { const order = []; const restoreArgs = []; const base = adapters({ order, restoreArgs }); const originalRead = base.readCurrent; let reads = 0; base.readCurrent = async expected => { const current = await originalRead(expected); reads += 1; if (reads === 2) { if (mismatch === 'etag') current.etag = '"badbadbadbadbadbadbadbadbadbadbadb"'; if (mismatch === 'versionId') current.versionId = 'other-version'; if (mismatch === 'document') current.document = { ...current.document, changed: true } } return current }; const result = await runDataRehearsal(base); assert.equal(result.status, 'failed'); assert.equal(result.counts.restores, 1); assert.equal(restoreArgs.length, 1); assert.equal(restoreArgs[0].ifMatch, '"0123456789abcdef0123456789abcdef"'); assert.equal(restoreArgs[0].versionId, 'test-version'); assert.equal(order.filter(stage => stage === 'restore').length, 1) }
})
...
test('restore proof independently rejects identity, readback, metadata, and content mismatches', () => {
  const doc = { schemaVersion: 1, stadium: 'oda', yearMonth: '2026-08', updatedAt: '2026-01-01T00:00:00.000Z', days: { '2026-08-09': [0, 1, 2], '2026-08-10': [1, 2, 0], '2026-08-11': [2, 0, 1], '2026-08-12': [0, 1, 2], '2026-08-13': [1, 2, 0], '2026-08-14': [2, 0, 1], '2026-08-15': [0, 1, 2] } }; const originalBytes = Buffer.from(`${JSON.stringify(doc, null, 2)}\n`); const original = { bytes: originalBytes, parsed: { document: doc }, head: { ETag: DATA_CONSTANTS.baselineEtag, VersionId: DATA_CONSTANTS.baselineVersionId } }; const response = { ETag: DATA_CONSTANTS.baselineEtag, VersionId: 'restore-version' }; const readback = { bytes: originalBytes, parsed: { document: doc, tuple: 0 }, head: { ETag: response.ETag, VersionId: response.VersionId, ContentType: DATA_CONSTANTS.contentType, CacheControl: DATA_CONSTANTS.cacheControl, ServerSideEncryption: 'AES256' } }; assert.equal(validateRestoreProof({ response, readback, original, testEtag: '"0123456789abcdef0123456789abcdef"', testVersionId: 'test-version' }), true)
  const cases = [
    { response: { ...response, ETag: 'weak' } }, { response: { ...response, ETag: undefined } }, { response: { ...response, ETag: '"0123456789abcdef0123456789abcdef"' } }, { response: { ...response, VersionId: '' } }, { response: { ...response, VersionId: undefined } }, { response: { ...response, VersionId: DATA_CONSTANTS.baselineVersionId } }, { response: { ...response, VersionId: 'test-version' } },
...
test('direct construction uses low-level CLI and independent browser ports', async () => {
  ...
  const execFile = async (_file, args) => {
    ...
    if (args[1] === 'get-object') { const bytes = currentEtag === '"0123456789abcdef0123456789abcdef"' ? Buffer.from(JSON.stringify({ ...baselineDocument, updatedAt: '2026-08-11T00:00:00.000Z', days: { ...baselineDocument.days, '2026-08-09': [1, 1, 2] } })) : baseline; await (await import('node:fs/promises')).writeFile(args.at(-1), bytes); return { stdout: '', stderr: '' } }
    if (args[1] === 'put-object') { currentEtag = DATA_CONSTANTS.baselineEtag; currentVersion = 'restore-version'; return { stdout: JSON.stringify({ ETag: currentEtag, VersionId: currentVersion }), stderr: '' } }
  ...
  const browser = { async setup() { return { contexts: 2 } }, async load() { return proof.load }, async update() { currentEtag = '"0123456789abcdef0123456789abcdef"'; currentVersion = 'test-version'; return proof.update }, async readCurrent() { return { state: 'test', etag: currentEtag, versionId: currentVersion, tuple: 1, document: { ...baselineDocument, updatedAt: '2026-08-11T00:00:00.000Z', days: { ...baselineDocument.days, '2026-08-09': [1, 1, 2] } } } }, async stale() { return proof.stale }, async poll(input) { return { tuple: input.expected, attempts: 1 } }, async cleanup() { return proof.cleanup } }
...
test('low-level Playwright page boundary observes exact UI PUT JSON response', async () => {
  ...
  let requestListener
  let savedCell = false; let cellValue = '0'; const cell = { waitFor: async () => {}, isVisible: async () => true, isEnabled: async () => !savedCell, inputValue: async () => savedCell ? '1' : cellValue, selectOption: async value => { assert.equal(value, '1'); cellValue = value }, click: async () => {} }
  const save = { waitFor: async () => {}, isVisible: async () => true, isEnabled: async () => true, click: async ({ trial } = {}) => { if (!trial) { savedCell = true; requestListener?.(request) } } }
  ...
  assert.equal(result.status, 200); assert.equal(result.saved, true); assert.equal(result.tuple, 1)
})
...
function submitFixture({ requestOverrides = {}, responseOverrides = {}, responseJson, conflict = false, conflictUi = true, conflictHeadingText = '別の更新があるため保存できませんでした。下に最新値を表示しています。', conflictHeadingVisible = true, conflictActionLabels = ['最新値へ再適用して編集を維持', '最新値で置き換え'], conflictActionVisible = true, conflictActionEnabled = true, conflictActionTrial = true, cellOptions = {}, saveOptions = {} } = {}) {
  ...
  let requestListener; let saveClicks = 0; let savedState = false
  const cellStarted = Date.now(); let cellValue = cellOptions.value ?? '0'; const cellEnabled = () => savedState ? cellOptions.postSaveEnabled ?? false : cellOptions.enableAfter === undefined ? cellOptions.enabled ?? true : Date.now() - cellStarted >= cellOptions.enableAfter; const cell = { waitFor: async () => { if (cellOptions.delay) await new Promise(resolve => setTimeout(resolve, cellOptions.delay)) }, isVisible: async () => savedState ? cellOptions.postSaveMissing ? false : cellOptions.postSaveVisible ?? true : cellOptions.visible ?? true, isEnabled: async () => cellEnabled(), inputValue: async () => savedState ? cellOptions.postSaveValue ?? cellValue : cellValue, selectOption: async value => { cellValue = value }, click: async ({ timeout = 1000 } = {}) => { const deadline = Date.now() + timeout; while (!cellEnabled() && Date.now() < deadline) await new Promise(resolve => setTimeout(resolve, 1)); if (!cellEnabled()) throw new Error('not actionable') } }; const saveStarted = Date.now(); const saveEnabled = () => saveOptions.enableAfter === undefined ? saveOptions.enabled ?? true : Date.now() - saveStarted >= saveOptions.enableAfter; const save = { waitFor: async () => { if (saveOptions.delay) await new Promise(resolve => setTimeout(resolve, saveOptions.delay)) }, isVisible: async () => saveOptions.visible ?? true, isEnabled: async () => saveEnabled(), click: async ({ trial = false, timeout = 1000 } = {}) => { const deadline = Date.now() + timeout; while (!saveEnabled() && Date.now() < deadline) await new Promise(resolve => setTimeout(resolve, 1)); if (!saveEnabled()) throw new Error('not actionable'); if (!trial) { saveClicks += 1; savedState = true; if (requestListener) requestListener(request) } } }
  const metadata = { waitFor: async () => {}, isVisible: async () => true }; const status = { waitFor: async () => {}, isVisible: async () => true, textContent: async () => 'Saved.' }; const conflictHeading = { waitFor: async () => {}, isVisible: async () => conflictHeadingVisible, textContent: async () => conflictHeadingText }; const conflictControlFor = name => { const matches = label => typeof name?.test === 'function' ? (name.lastIndex = 0, name.test(label)) : true; return { count: async () => conflictUi ? conflictActionLabels.filter(matches).length : 0, waitFor: async () => {}, isVisible: async () => conflictUi && conflictActionVisible, isEnabled: async () => conflictUi && conflictActionEnabled, click: async ({ trial } = {}) => { assert.equal(trial, conflictActionTrial); if (!trial) throw new Error('real click forbidden') } } }; const page = { locator: selector => { if (selector === '.admin-metadata') return metadata; if (selector === '#conflict-title') return conflictHeading; assert.equal(selector, '[id="2026-08-09-0"]'); return cellOptions.missing || (savedState && cellOptions.postSaveMissing) ? {} : cell }, on: (event, listener) => { if (event === 'request') requestListener = listener }, off: () => { requestListener = null }, waitForRequest: async () => request, waitForResponse: async predicate => { responseCount += 1; if (conflict && responseCount === 2) return { status: () => 200, url: () => `${DATA_CONSTANTS.apiOrigin}${DATA_CONSTANTS.apiPath}`, request: () => ({ method: () => 'GET' }), headers: () => ({ 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' }) }; return response }, getByRole: (role, options = {}) => role === 'status' ? status : role === 'alert' ? alertNone() : role === 'button' && isSaveName(options.name) ? (saveOptions.missing ? {} : save) : conflictControlFor(options.name) }
...
test('cell and localized Save readiness is bounded and fails before any PUT', async () => {
  ...
  for (const [name, cellOptions] of [['enabled', { postSaveEnabled: true }], ['hidden', { postSaveVisible: false }], ['wrong-value', { postSaveValue: '0' }], ['missing', { postSaveMissing: true }]]) {
    const fixture = submitFixture({ cellOptions })
    await assert.rejects(browser.submit(fixture.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: fixture.baseline }, 1), /saved target cell/, name)
    assert.equal(fixture.saveClicks(), 1)
  }
...
test('low-level stale conflict resolves with one PUT, comparison GET, and localized UI', async () => {
  ...
  const conflictSave = { waitFor: async () => {}, isVisible: async () => true, isEnabled: async () => true, click: async ({ trial } = {}) => { if (!trial) registered?.(conflict.page.requestForTest) } }; const conflictControl = { count: async () => 1, waitFor: async () => {}, isVisible: async () => true, isEnabled: async () => true, textContent: async () => '別の更新があるため保存できませんでした。下に最新値を表示しています。', click: async ({ trial } = {}) => { assert.equal(trial, true) } }; conflict.page.getByRole = (role, options) => isSaveName(options.name) ? conflictSave : conflictControl
...
test('conflict heading and action wait boundedly, including delayed success and timeout', async () => {
  ...
  const delayedControl = delay => ({ count: async () => 1, waitFor: async ({ timeout }) => { if (delay > timeout) { await new Promise(resolve => setTimeout(resolve, timeout)); throw new Error('timeout') } await new Promise(resolve => setTimeout(resolve, delay)) }, isVisible: async () => true, isEnabled: async () => true, textContent: async () => '別の更新があるため保存できませんでした。下に最新値を表示しています。', click: async ({ trial } = {}) => { assert.equal(trial, true) } })
...
test('conflict heading accepts exact Japanese and English locales only', async () => {
  const browser = createPlaywrightDataBrowser({ launcher: async () => { throw new Error('not used') } })
  for (const text of ['別の更新があるため保存できませんでした。下に最新値を表示しています。', 'The schedule changed elsewhere. Your draft is preserved below.']) {
    const fixture = submitFixture({ conflict: true, conflictHeadingText: text })
    assert.equal((await browser.submit(fixture.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: fixture.baseline }, 1, true)).conflict, true)
  }
  for (const options of [{ conflictHeadingText: '競合' }, { conflictHeadingText: '' }, { conflictHeadingVisible: false }, { missing: true }]) {
    const fixture = submitFixture({ conflict: true, ...(options.missing ? {} : options) }); if (options.missing) { const locator = fixture.page.locator; fixture.page.locator = selector => selector === '#conflict-title' ? {} : locator(selector) }
    await assert.rejects(browser.submit(fixture.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: fixture.baseline }, 1, true), /conflict heading/)
  }
})

test('conflict action selects the unique localized rebase button without a real click', async () => {
  const browser = createPlaywrightDataBrowser({ launcher: async () => { throw new Error('not used') } })
  for (const conflictActionLabels of [['最新値へ再適用して編集を維持', '最新値で置き換え'], ['Keep edits on latest', 'Replace with latest']]) {
    const fixture = submitFixture({ conflict: true, conflictActionLabels })
    assert.equal((await browser.submit(fixture.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: fixture.baseline }, 1, true)).conflict, true)
  }
  for (const options of [
    { conflictActionLabels: [] },
    { conflictActionLabels: ['最新値へ再適用して編集を維持', '最新値へ再適用して編集を維持'] },
    { conflictActionLabels: ['Wrong action'] },
    { conflictActionVisible: false },
    { conflictActionEnabled: false },
    { conflictActionTrial: false },
  ]) {
    const fixture = submitFixture({ conflict: true, ...options })
    await assert.rejects(browser.submit(fixture.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: fixture.baseline }, 1, true), /conflict action/)
  }
})
...
test('submit avoids response body reads and rejects missing conflict controls', async () => {
  const timestamp = submitFixture(); timestamp.page.waitForResponse = async () => ({ status: () => 200, url: () => `${DATA_CONSTANTS.apiOrigin}${DATA_CONSTANTS.apiPath}`, request: () => ({ method: () => 'PUT' }), headers: () => ({ 'content-type': 'application/json', 'cache-control': 'no-store' }), json: async () => { throw new Error('response body must not be read') } })
  const browser = createPlaywrightDataBrowser({ launcher: async () => { throw new Error('not used') } })
  ...
  await assert.rejects(browser.submit(missing.page, { ifMatch: DATA_CONSTANTS.baselineEtag, baselineDocument: missing.baseline }, 1, true), /conflict heading|conflict action|conflict UI missing/)
})
