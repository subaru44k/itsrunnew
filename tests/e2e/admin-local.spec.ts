...
test('B2 stale conflict preserves diff and explicit rebase uses latest ETag', async ({ page }) => {
  ...
  await signInForB2(page, english); await page.locator('.admin-table select').first().selectOption('1'); await page.getByRole('button', { name: english ? 'Save' : '保存' }).click(); await expect(page.getByText(english ? 'The schedule changed elsewhere.' : '別の更新があるため保存できませんでした。下に最新値を表示しています。')).toBeVisible(); expect(gets).toBe(2); expect(puts).toBe(1); await expect(page.getByRole('button', { name: english ? 'Save' : '保存' })).toHaveCount(0); await expect(page.locator('body')).toContainText(english ? 'Unknown' : '未公開'); await expect(page.locator('body')).toContainText(english ? 'Available' : '利用可能'); await expect(page.locator('body')).toContainText(english ? 'Unavailable' : '利用不可'); await expect(page.locator('body')).not.toContainText('raw-conflict-body')
  await page.getByRole('button', { name: english ? 'Keep edits on latest' : '最新値へ再適用して編集を維持' }).click(); await page.locator('.admin-table select').first().selectOption('1'); await page.getByRole('button', { name: english ? 'Save' : '保存' }).click(); await expect(page.getByText(english ? 'Saved.' : '保存しました。')).toBeVisible(); expect(puts).toBe(2); await expect(page.locator('body')).toContainText('saved-version')
})
...
test('B2 comparison failure exposes GET-only retry and then latest actions', async ({ page }) => {
  ...
  await signInForB2(page, english); await page.locator('.admin-table select').first().selectOption('1'); await page.getByRole('button', { name: english ? 'Save' : '保存' }).click(); await expect(page.getByRole('button', { name: english ? 'Retry latest comparison' : '最新値の取得を再試行' })).toBeVisible(); expect(gets).toBe(2); expect(puts).toBe(1); await expect(page.locator('body')).toContainText(english ? 'The latest schedule could not be loaded.' : '最新データを取得できませんでした。'); await expect(page.locator('body')).not.toContainText(/raw-comparison-body|raw-conflict-body/); await page.getByRole('button', { name: english ? 'Retry latest comparison' : '最新値の取得を再試行' }).click(); await expect(page.getByRole('button', { name: english ? 'Keep edits on latest' : '最新値へ再適用して編集を維持' })).toBeVisible(); expect(gets).toBe(3); expect(puts).toBe(1)
})
...
test('B2 dirty month and stadium changes require confirmation and exact new GET', async ({ page }) => {
  ...
  await signInForB2(page, english); await page.locator('.admin-table select').first().selectOption('1'); const confirm = english ? 'Discard unsaved changes?' : '未保存の変更を破棄しますか？'; await page.locator('.admin-filters select').selectOption('yumenoshima'); const load = page.getByRole('button', { name: english ? 'Load' : '読み込む' }); page.once('dialog', async (dialog) => { expect(dialog.message()).toBe(confirm); await dialog.dismiss() }); await load.click(); expect(gets).toBe(1); await expect(page.locator('.admin-filters select')).toHaveValue('oda'); await page.locator('.admin-filters select').selectOption('yumenoshima'); page.once('dialog', async (dialog) => { expect(dialog.message()).toBe(confirm); await dialog.accept() }); await load.click(); await expect(page.locator('.admin-filters select')).toHaveValue('yumenoshima'); expect(gets).toBe(2); expect(requested).toContain('/stadiums/yumenoshima/availability/2026-08')
})
