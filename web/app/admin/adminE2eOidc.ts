...
export function createAdminE2eOidcPort(): OidcPort {
  ...
  return {
    ...
    signinCallback: async (url) => { count('admin-e2e-callback-count'); const callback = typeof url === 'string' ? new URL(url) : null; if (url !== window.location.href && !(callback?.origin === window.location.origin && callback.pathname === '/manage/callback' && callback.search !== '')) throw new Error('callback URL mismatch'); if (mode() === 'callbackFailure') throw new Error('fake callback failure'); user = fakeUser({ returnPath: mode() === 'hostileReturn' ? 'https://evil.invalid' : '/manage' }); handlers.forEach((handler) => handler(user as User)); return user as User },
