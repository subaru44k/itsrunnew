...
<template>
  <div class="schedule-wrap" :aria-busy="loading ? 'true' : 'false'">
    ...
    <table class="schedule-table">
      ...
      <tbody><tr v-for="date in dates" :key="date"><th scope="row">{{ date }}</th><td v-for="slot in 3" :key="slot"><span v-if="statusFor(date, slot - 1) === null" class="status status-unknown"><span class="status-symbol" aria-hidden="true">?</span>{{ $t('scheduleUnknown') }}</span><span v-else :class="`status status-${statusFor(date, slot - 1)}`"><span class="status-symbol" aria-hidden="true">{{ statusFor(date, slot - 1) === 1 ? '○' : '×' }}</span>{{ statusLabel(statusFor(date, slot - 1)!, locale === 'ja' ? 'ja' : 'en') }}</span></td></tr></tbody>
    </table>
  </div>
</template>
