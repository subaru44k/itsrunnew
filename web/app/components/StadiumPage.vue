...
<template><article class="content" v-if="stadium"><p class="eyebrow">{{ $t('siteName') }}</p><h1>{{ $t(stadium.nameKey) }}</h1><p>{{ $t('homeDescription') }}</p><h2>{{ $t('scheduleTitle') }}</h2><p v-if="schedule.state === 'error'" role="alert">{{ schedule.error }} <button type="button" @click="schedule.load">Retry</button></p><div class="week-controls"><button type="button" @click="schedule.previousWeek">Previous week</button><button type="button" @click="schedule.nextWeek">Next week</button></div><ScheduleTable :dates="schedule.dates.value" :months="schedule.months.value" :loading="schedule.state === 'loading'" /></article></template>
