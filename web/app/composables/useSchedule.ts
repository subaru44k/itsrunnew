...
import { HttpScheduleRepository } from '~/repositories/httpScheduleRepository'
...
export function useSchedule(stadium: StadiumSlug) {
  ...
  async function load() {
    ...
    try {
      ...
      if (current !== requestId) return
      state.value = 'error'; error.value = reason instanceof Error ? reason.message : 'Unable to load schedule'
    }
  }
  ...
  return { dates, months, state, error, load, previousWeek, nextWeek }
}
